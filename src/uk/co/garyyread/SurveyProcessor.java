...
public class SurveyProcessor {
    ...
    public boolean loadWorkbook(String loc) {
        ...
        try {
            ...
            if (debug) debug("readWorkBook(" + loc + ")");
            result = true;
    ...
    public HashMap<String, String[]> processSheet(String name) {
        ...
        if (workingSheet == null) {
            ...
            if (debug) debug("EXCEPTION:processSheet(" + name + "),sheet does not exist.");
            return null;
        }
        ...
        for (int i = ROW_START; i < data[ID_POS].length && data[ID_POS][i].getContents().length() > 0; i++) {
            ...
            //Compare date if key exists (find eariest date) then add to HashMap
                ...
                if (dateStrOld.length() > 0 && ageClass != -1) dateOld = LocalDate.parse(dateStrOld);
                ...
                if (dateOld == null && ageClass > -1) {
                    ...
                    if (debug) debug("ADDED TO EXISTING" + TAB + Arrays.toString(arrOld) + " to " + Arrays.toString(arrNew));
                ...
                //Compare dates and add the earliest occurance of the seal
                    ...
                    if (dateOld.compareTo(date) > 0) {
                        ...
                        if (debug) debug("UPDATED" + TAB + Arrays.toString(arrOld) + " to " + Arrays.toString(arrNew));
                    }
                }
            ...
            //New item, add straight to the array
                ...
                if (debug) debug("NEW" + TAB + Arrays.toString(arr));
            }
        }
    ...
    /**
     * Not all cell values are regular, take last char from string (always number)
    ...
    private LocalDate convertStrToDate(String dateStr) {
        ...
        if (debug) debug("convertStrToDate(" + dateStr + ")");
                ...
                for (int c = 0; arr != null && c < arr.length; c++) {
                    ...
                    if (c == 1) {
                        ...
                        try {
                            ...
                            if (sp.debug) sp.debug("EXCEPTION:method(),No number found in data \"" + cont + "\"");
                        }
                        
                    //date cell
                    } else if (c > 2 && c < 9) {
                        try {
                            if (cont.length() > 0) ws.addCell(new Label(altC, r, cont));
                        } catch (StringIndexOutOfBoundsException ex) {
                            if (sp.debug) sp.debug("EXCEPTION:method(),No date found in data \"" + cont + "\"");
                        }
                    ...
                    //All other columns (minus colour information)
                        ...
                        if (c != 2) ws.addCell(new Label(c, r, cont));
                    }
                }
            }
